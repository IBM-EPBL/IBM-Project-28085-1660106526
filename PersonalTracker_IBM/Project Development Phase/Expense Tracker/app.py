...
def index():
    ...
    if request.method == "GET":
        ...
        categories = categories.getSpendCategories(session["user_id"])

        # Get the users payers (for quick expense modal)
        payers = account.getPayers(session["user_id"])

        # Get todays date (for quick expense modal)
        date = datetime.today().strftime('%Y-%m-%d')

        # Get the users income
        income = account.getIncome(session["user_id"])

        # Get current years total expenses for the user
        expenses_year = dashboard.getTotalSpend_Year(session["user_id"])

        # Get current months total expenses for the user
        expenses_month = dashboard.getTotalSpend_Month(
            session["user_id"])

        # Get current week total expenses for the user
        expenses_week = dashboard.getTotalSpend_Week(session["user_id"])

        # Get last 5 expenses for the user
        expenses_last5 = dashboard.getLastFiveExpenses(
            session["user_id"])

        # Get every budgets spent/remaining for the user
        budgets = dashboard.getBudgets(session["user_id"])

        # Get weekly spending for the user
        weeks = dashboard.getLastFourWeekNames()
        spending_week = dashboard.getWeeklySpending(
            weeks, session["user_id"])

        # Get monthly spending for the user (for the current year)
        spending_month = dashboard.getMonthlySpending(
            session["user_id"])

        # Get spending trends for the user
        spending_trends = dashboard.getSpendingTrends(
            session["user_id"])

        # Get payer spending for the user
        payersChart = reports.generatePayersReport(session["user_id"])
    ...
    # User reached route via POST
        ...
        expenses = addingExpenses.addExpenses(formData, session["user_id"])
...
def expensehistory():
    ...
    if request.method == "GET":
        ...
        history = addingExpenses.getHistory(session["user_id"])
    ...
    # User reached route via POST
        ...
        oldExpense = addingExpenses.getExpense(
        ...
        if userHasSelected_deleteExpense == True:
            ...
            deleted = addingExpenses.deleteExpense(
            ...
            history = addingExpenses.getHistory(session["user_id"])
            categories = categories.getSpendCategories(
                session["user_id"])
            payers = account.getPayers(session["user_id"])
        ...
        # Update the existing expense record
            ...
            expensed = addingExpenses.updateExpense(
...
def budgets(year=None):
    ...
    if request.method == "GET":
        ...
        budgets = budget.getBudgets(session["user_id"])

        # Get the users total budgeted amount
        budgeted = budget.getTotalBudgetedByYear(
    ...
    # User reached route via POST
        ...
        # Delete the budget
        deletedBudgetName = budget.deleteBudget(
...
def createbudget():
    ...
    if request.method == "POST":
        ...
        budgetDict = budget.generateBudgetFromForm(formData)
        ...
        # Get the users total budgeted amount
        budgeted = budget.getTotalBudgetedByYear(session["user_id"])
...
def updatebudget(urlvar_budgetname):
    ...
    # User reached route via GET
        ...
        if budgetID is None:
            ...
            budget = budget.getBudgetByID(budgetID, session["user_id"])
        ...
        budget = budget.getUpdatableBudget(budget, session["user_id"])
...
def categories():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_newCategory:
            ...
            categoryCount = len(
                categories.getSpendCategories(session["user_id"]))
            if categoryCount >= 30:
            ...
            categoryID = categories.getCategoryID(newCategoryName)
            ...
            if categoryID:
                ...
                existingID = categories.getCategoryID(
            ...
            # Category does not exist in the DB already - create a new category and then add it to the users account
                ...
                newCategoryID = categories.addCategory_DB(
                    newCategoryName)
        ...
        if userHasSelected_renameCategory:
            ...
            oldCategoryID = categories.getCategoryID(oldCategoryName)
            ...
            # Check to see if the *new* name already exists in the database (None == does not exist)
            newCategoryID = categories.getCategoryID(newCategoryName)
            ...
            if newCategoryID:
                ...
                newCategoryNameFromDB = categories.getSpendCategoryName(
                    newCategoryID)
        ...
        if userHasSelected_deleteCategory:
            ...
            categoryID = categories.getCategoryID(deleteName)
            ...
            # Delete the category
            categories.deleteCategory(categoryID, session["user_id"])
    ...
    # User reached route via GET
        ...
        categoryBudgets = categories.getBudgetsSpendCategories(
            session["user_id"])

        # Generate a single data structure for storing all categories and their associated budgets
        categoriesWithBudgets = categories.generateSpendCategoriesWithBudgets(
...
def budgetsreport(year=None):
    ...
    budgets = reports.generateBudgetsReport(session["user_id"], year)
...
def monthlyreport(year=None):
    ...
    monthlySpending = reports.generateMonthlyReport(
        session["user_id"], year)
...
def spendingreport(year=None):
    ...
    spendingReport = reports.generateSpendingTrendsReport(
        session["user_id"], year)
...
def payersreport(year=None):
    ...
    payersReport = reports.generatePayersReport(
        session["user_id"], year)
...
def updateaccount():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_updateIncome:
            ...
            updatedIncome = account.updateIncome(
        ...
        if userHasSelected_addPayer:
            ...
            # Add the payer
            newPayer = account.addPayer(newName, session["user_id"])
        ...
        if userHasSelected_renamePayer:
            ...
            # Rename the payer
            renamedPayer = account.renamePayer(
        ...
        if userHasSelected_deletePayer:
            ...
            # Delete the payer
            deletedPayer = account.deletePayer(name, session["user_id"])
        ...
        if userHasSelected_updatePassword:
            ...
            updatedPassword = account.updatePassword(request.form.get(
        ...
        user = account.getAllUserInfo(session["user_id"])
